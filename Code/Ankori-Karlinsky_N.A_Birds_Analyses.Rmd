...
library(blmeco)  #Testing overdispersion
library(MuMIn) #Model results and interpretation
library(lme4) #mixed models
library(broom.mixed) #accounting for overdispersion
library(optimx) #optimize models
library(glmmTMB) #rsquared and fitting negative binomial
library(DHARMa) #plotting residuals for negative binomial
...
library(rstatix) #t-test assumptions
library(pixiedust) #rendering in markdown
...
It is also tested by performing a non-parametric dependent 2-group Wilcoxon Signed Rank Test
...
```{r Prediction 1 T-test assumption tests, message=FALSE, warning=FALSE}

#Running shapiro-wilks test of normality of differences in means

#Precipitation range
prec_shapiro<-Environmental_Range_Comparison %>% 
  group_by(State) %>% #grouping by state
  summarize(prec_diff = Precipitation_Range - lag(Precipitation_Range)) %>% #difference BBA-BBS
  filter(!is.na(prec_diff))#remove NA

shapiro_test(as.data.frame(prec_shapiro),prec_diff) #shaprio-wilks test (p value above 0.05 means we can do t-test)

#Temperature range
temp_shapiro<-Environmental_Range_Comparison %>% 
  group_by(State) %>% #grouping by state
  summarize(temp_diff = Temperature_Range - lag(Temperature_Range)) %>% #difference BBA-BBS
  filter(!is.na(temp_diff))#remove NA

shapiro_test(as.data.frame(temp_shapiro),temp_diff) #shaprio-wilks test (p value above 0.05 means we can do t-test)

#Elevation range
elev_shapiro<-Environmental_Range_Comparison %>% 
  group_by(State) %>% #grouping by state
  summarize(elev_diff = Elevation_Range - lag(Elevation_Range)) %>% #difference BBA-BBS
  filter(!is.na(elev_diff))#remove NA

shapiro_test(as.data.frame(elev_shapiro),elev_diff) #shaprio-wilks test (p value above 0.05 means we can do t-test)

#all good!
rm(prec_shapiro,temp_shapiro,elev_shapiro)
...
```{r Prediction 1 Wilcox Test All Data,message=FALSE, warning=FALSE}

#For entire data Precipitation
wilcox.test(Precipitation_Range~Dataset,
            data=Environmental_Range_Comparison,
            paired=T,
            alternative="greater",
            conf.int = T)

#For entire data Temperature
wilcox.test(Temperature_Range~Dataset,
            data=Environmental_Range_Comparison,
            paired=T,
            alternative="greater",
            conf.int = T)
...
#For entire data Elevation
wilcox.test(Elevation_Range~Dataset,
            data=Environmental_Range_Comparison,
            paired=T,
            alternative="greater",
            conf.int = T)
...
```{r Prediction 1 Wilcox Test Adjacent,message=FALSE, warning=FALSE}

#For entire data Precipitation
wilcox.test(Precipitation_Range~Dataset,
            data=Adjacent_Environmental_Range_Comparison,
            paired=TRUE,
            alternative="greater",
            conf.int = T)
...
```{r Prediction 1 Wilcox Test Sampled,message=FALSE, warning=FALSE}

#For entire data Precipitation
wilcox.test(Precipitation_Range~Dataset,
            data=Sampled_Environmental_Range_Comparison,
            paired=TRUE,
            alternative="greater",
            conf.int = T)
...
```{r Prediction 1 Wilcox Test Sampled Adjacent,message=FALSE, warning=FALSE}

#For entire data Precipitation
wilcox.test(Precipitation_Range~Dataset,
            data=Sampled_Adjacent_Environmental_Range_Comparison,
            paired=TRUE,
            alternative="greater",
            conf.int = T)
...
rm(Environmental_Range_Comparison, Adjacent_Environmental_Range_Comparison,
   ...
   Env.BBA,Env.BBS,Env.BBA,Adj.Env.BBA,Adj.Env.BBS,Sampled.Env.BBA,Sampled.Env.BBS,
   Sampled.Adj.Env.BBA,Sampled.Adj.Env.BBS)
...
Because elevation is right skewed, we use gamma distribution

```{r Prediction 2 Elevation Range All data, message=FALSE, warning=FALSE}

#running model gamma distribution non-log transformed
prediction2.elev <- full_d %>%
  filter(Elev_Range != 0) %>%
  glmer(Elev_Range ~ as.factor(Dataset)
                          + (1|State),
                          data=.,
...
#anova
anova.elev_range<- aov(Elev_Range ~ Dataset, data=full_d)
summary(anova.elev_range)
TukeyHSD(anova.elev_range)


#running model (assumed gaussian) log transformed
prediction2b.elev <- full_d %>%
  mutate(log_range = log1p(Elev_Range)) %>% #log-transforming
  filter(!is.na(log_range),
         log_range != "Inf",
         log_range != "-Inf") %>% #removing infinity values
  lmer(log_range ~ as.factor(Dataset) #running model
                              + (1|State),
                              data=.)

#table output
tab_model(prediction2b.elev,
          show.est = T)
...
full_d %>%
  group_by(State) %>%
  do(fitElev = glance(lm(Elev_Range ~ as.factor(Dataset), 
                       data = .))) %>%
  unnest(fitElev)
...
```{r prediction 2 elevation range all data plots, message=FALSE, warning=FALSE}
...
#plotting diagnostic
plot_model(prediction2.elev, type="diag")
plot(prediction2.elev)
#plot
qqnorm(resid(prediction2.elev))
qqline(resid(prediction2.elev))

#plotting diagnostic log-transformed
plot_model(prediction2b.elev, type="diag")
plot(prediction2b.elev)
#plot
qqnorm(resid(prediction2b.elev))
qqline(resid(prediction2b.elev))

#bartlett test for homogeneity of variance
bartlett.test(Elev_Range ~ as.factor(Dataset),
              data=full_d)

#removing from environment to save space
rm(prediction2.elev,prediction2b.elev)
...
prediction2.landrich <- lmer(Het_Count2006 ~ as.factor(Dataset)
                              +(1|State),
                              data=full_d)
...
#anova
anova.landrich <- aov(Het_Count2006 ~ Dataset, data=full_d)
summary(anova.landrich)
TukeyHSD(anova.landrich)


#running model Poisson distribution for count data
prediction2b.landrich <- glmer(Het_Count2006 ~ as.factor(Dataset)
                              +(1|State),
                              data=full_d,
                              family=poisson(link="log"))

#output model table
tab_model(prediction2b.landrich,
          title="Prediction 2 Poisson",
          show.se=T)

#using the glmmTMB pacakge to calculate quasi-possion (i.e. negative binomial 1)
prediction2c.landrich <-glmmTMB::glmmTMB(Het_Count2006 ~ as.factor(Dataset)+
                      (1|State),
                       data=full_d,
                       family="nbinom1") #negative binoimal 1 or quasi-poisson

#table of model
tab_model(prediction2c.landrich,
          title="Prediction 2 Quasi-Poisson",
...
```{r Prediction 2 Land-Class Richness Entire Dataset Plots}
...
#plotting diagnostic
plot_model(prediction2.landrich, type="diag")
plot(prediction2.landrich)
qqnorm(resid(prediction2.landrich))
qqline(resid(prediction2.landrich))
plot(resid(prediction2.landrich),full_d$Het_Count2006)


#plotting diagnostic
plot_model(prediction2b.landrich, type="diag")
plot(prediction2b.landrich)
qqnorm(resid(prediction2b.landrich))
qqline(resid(prediction2b.landrich))
plot(resid(prediction2b.landrich),full_d$Het_Count2006)

#plotting diagnostic
plot_model(prediction2c.landrich, type="diag")
#plot(prediction2c.landrich)
qqnorm(resid(prediction2c.landrich))
qqline(resid(prediction2c.landrich))
plot(resid(prediction2c.landrich),full_d$Het_Count2006)
plot(full_d$Dataset,resid(prediction2c.landrich))


#removing from environment to save space
rm(prediction2.landrich,prediction2b.landrich,prediction2c.landrich)
...
#running model (assumed gaussian)
prediction2.landdiv <- lmer(Shannon2006 ~ as.factor(Dataset)
                              + (1|State),
                              data=full_d)

#anova
anova.landdiv <- aov(Shannon2006 ~ Dataset, data=full_d)
summary(anova.landdiv)
TukeyHSD(anova.landdiv)
...
#bartlett test
bartlett.test(Shannon2006 ~ as.factor(Dataset),
              data=full_d)

```


```{r Prediction 2 Land-Class Diversity Entire Data plots, warning=FALSE, message=FALSE}
#extracting r2
...
#plot diagnostic
plot_model(prediction2.landdiv, type="diag")
plot(prediction2.landdiv)
qqnorm(resid(prediction2.landdiv))
qqline(resid(prediction2.landdiv))
...
#plot diagnostics
plot_model(adj_prediction2.elev,type="diag")
plot(adj_prediction2.elev)
qqnorm(resid(adj_prediction2.elev))
qqline(resid(adj_prediction2.elev))
...
RichBBA <- Richness_Comparison[1:5,]
RichBBS <- Richness_Comparison[6:10,]
...
```{r Prediction 3 T-Test Assumption Tests, message=FALSE, warning=FALSE}

#shapiro-wilk test of normality of difference in regional richness
richness_diff <- Richness_Comparison %>%
  group_by(State) %>% #grouping
  summarize(rich_diff = Regional_Richness - lag(Regional_Richness)) %>%
  filter(!is.na(rich_diff))

shapiro_test(as.data.frame(richness_diff),rich_diff)

```



```{r Prediction 3 T-Tests and Wilcox All Data,message=FALSE, warning=FALSE}
...
#Non-parametric test
wilcox.test(RichBBA$Regional_Richness,
            RichBBS$Regional_Richness,
            paired=TRUE,
            alternative="greater",
            conf.int = T)
...
```{r Prediction 3 T-Tests and Wilcox  Adjacent,message=FALSE, warning=FALSE}
...
```{r Prediction 3 T-Tests and Wilcox  Sampled,message=FALSE, warning=FALSE}
...
```{r Prediction 3 T-Tests and Wilcox Sampled Adjacent,message=FALSE, warning=FALSE}
...
tab_model(prediction6,title="Local Bird Richness by Dataset GLMM Poisson",
          string.pred = "Coefficient",
          string.ci = "95% CIs",
          string.p = "P-Value",
          show.se = T,
...
#adjusting model coefficients with this
...
kable(tidy_quasi(prediction6))


#---

#Now with normal distribution
prediction6a <- lmer(Atlas2_Total ~ Dataset+
                      (1|State),
               data=full_d)

#Output of regression as table
tab_model(prediction6a,title="Local Bird Richness by Dataset LMM",
          string.pred = "Coefficient",
          string.ci = "95% CIs",
          string.p = "P-Value",
          show.se = T,
...
#using the glmmTMB pacakge to calculate quasi-possion (i.e. negative binomial 1)
prediction6b <-glmmTMB::glmmTMB(Atlas2_Total ~ Dataset+
                      (1|State),
                       data=full_d,
                       family="nbinom1") #negative binoimal 1 or quasi-poisson

#table of model
tab_model(prediction6b,
          title="Local Bird Richness by Environment GLMM Quasi-Poisson",
          string.pred = "Coefficient",
          string.ci = "95% CIs",
          string.p = "P-Value",
          show.se = T,
...
```{r Prediction 6 GLMM Poisson Plots, message=FALSE, warning=FALSE}
...
#now plotting diagnostic
plot_model(prediction6, type="diag")
plot(prediction6)

qqnorm(resid(prediction6,type="deviance"))
qqline(resid(prediction6))

```


```{r Prediction 6a LMM Plots, message=FALSE, warning=FALSE}

#now plotting the rate ratios between the datasets with p-values
plot_model(prediction6a,show.values=T,show.p=T)+theme_classic()+
  labs(title="Local Bird Richness by Dataset",
       y="Rate Ratio (BBA to BBS)")+
  theme(plot.title=element_text(hjust=.5))+
  geom_hline(yintercept = 1,linetype="dotted")
...
#extracting r2
r2.c=round(as.numeric(rsq(prediction6a)[3]),digits=4)
r2.m=round(as.numeric(rsq(prediction6a)[2]),digits=4)

#now plotting the effect with richness on the y-axis
plot_model(prediction6a,type="pred",terms ="Dataset",
           show.values = T,
           show.p=T)+theme_classic()+
  labs(title="Local Bird Richness by Dataset from Model\n(State as random effect)",
       x="Dataset",
...
#now plotting the impact of the random effect of state
plot_model(prediction6a,type="re",
           show.values = T,
           show.p=T)+theme_classic()+
  labs(title="Random Effect of State on Species Richness Differences\n(BBA to BBS)",
       y="Rate Ratio (BBA to BBS)",
...
#now plotting predict with state
plot_model(prediction6a,type="pred",terms =c("State","Dataset"),
           show.values = T,
           show.p=T)+theme_classic()+
  labs(title="Local Bird Richness by Dataset from Model\n(State as random effect)",
       x="Dataset",
...
#now plotting diagnostic
plot_model(prediction6a, type="diag")
plot(prediction6a)

qqnorm(resid(prediction6a))
qqline(resid(prediction6a))

```

```{r Prediction 6b simple GLMM Quasi Poisson Plots, message=FALSE, warning=FALSE}

#now plotting the rate ratios between the datasets with p-values
plot_model(prediction6b,show.values=T,show.p=T)+theme_classic()+
  labs(title="Local Bird Richness by Dataset",
       y="Rate Ratio (BBA to BBS)")+
  theme(plot.title=element_text(hjust=.5))+
  geom_hline(yintercept = 1,linetype="dotted")
...
#extracting r2
r2.c=round(r.squaredGLMM(prediction6b)[1,2],digits=4)
r2.m=round(r.squaredGLMM(prediction6b)[1,1],digits=4)

#now plotting the effect with richness on the y-axis
plot_model(prediction6b,type="pred",terms ="Dataset",
           show.values = T,
           show.p=T)+theme_classic()+
  labs(title="Local Bird Richness by Dataset from Model\n(State as random effect)",
       x="Dataset",
...
#now plotting the impact of the random effect of state
plot_model(prediction6b,type="re",
           show.values = T,
           show.p=T)+theme_classic()+
  labs(title="Random Effect of State on Species Richness Differences\n(BBA to BBS)",
       y="Rate Ratio (BBA to BBS)",
...
#now plotting predict with state
plot_model(prediction6b,type="pred",terms =c("State","Dataset"),
           show.values = T,
           show.p=T)+theme_classic()+
  labs(title="Local Bird Richness by Dataset from Model\n(State as random effect)",
       x="Dataset",
...
#now plotting diagnostic
plot_model(prediction6b, type="diag")
#plot(prediction6b)

qqnorm(resid(prediction6b))
qqline(resid(prediction6b))
...
sampled.adj.prediction6<- lmer(Atlas2_Total ~ Dataset+
                      (1|State),
               data=SampledAdj_Full)
...
```{r Prediction 6 full GLMM, message=FALSE, warning=FALSE}
...
tab_model(prediction6.2,
          title="Local Bird Richness by Dataset GLMM Poisson",
          string.pred = "Coefficient",
          string.ci = "95% CIs",
          string.p = "P-Value",
          show.se = T,
...
kable(tidy_quasi(prediction6.2))
...
#Gaussian LMM 
prediction6.2a <- lmer(Atlas2_Total ~ 
                       Dataset   #ata source and effort
                       +scale(Precipitation)  #precipitation
                       +scale(Temp)  #summer temperature
                       +scale(Elev_Mean)  #elevation mean in block/route
...
#Output of regression as table
tab_model(prediction6.2a,
          title="Local Bird Richness by Dataset LMM",
          string.pred = "Coefficient",
          string.ci = "95% CIs",
          string.p = "P-Value",
          show.se = T,
...
#using the glmmTMB pacakge to calculate quasi-possion (i.e. negative binomial 1)
prediction6.2b <-glmmTMB::glmmTMB(Atlas2_Total ~ 
                       Dataset   #ata source and effort
                       +scale(Precipitation)  #precipitation
                       +scale(Temp)  #summer temperature
                       +scale(Elev_Mean)  #elevation mean in block/route
...
#table of model
tab_model(prediction6.2b,
          title="Local Bird Richness by Environment Full GLMM Quasi-Poisson",
          string.pred = "Coefficient",
          string.ci = "95% CIs",
          string.p = "P-Value",
          show.se = T,
...
```{r Prediction 6 full GLMM Poisson plots, message=FALSE, warning=FALSE}
...
#now plotting diagnostic
plot(prediction6.2)
plot_model(prediction6.2, type="resid")
plot_model(prediction6.2, type="diag")
qqnorm(residuals(prediction6.2,type="deviance"))
qqline(resid(prediction6.2))


```

```{r Prediction 6 full LMM plots, message=FALSE, warning=FALSE}

#Plotting model
plot_model(prediction6.2a,show.values = T,show.p=T,show.intercept = T)+theme_classic()+
  labs(title="GLM All Factors Contributing to Local Richness")

#extracting r2
r2.c=round(as.numeric(rsq(prediction6.2a)[3]),digits=4)
r2.m=round(as.numeric(rsq(prediction6.2a)[1]),digits=4)

#now plotting the effect with richness on the y-axis
plot_model(prediction6.2a,type="pred",terms ="Dataset",
           show.values = T,
           show.p=T)+theme_classic()+
  labs(title="Local Bird Richness by Dataset Full GLM\n(State as random effect)",
       x="Dataset",
...
#now plotting the impact of the random effect of state
plot_model(prediction6.2a,type="re",
           show.values = T,
           show.p=T)+theme_classic()+
  labs(title="Random Effect of State Full GLM\n(BBA to BBS)",
       y="Rate Ratio (BBA to BBS)",
...
#now plotting predict with state
plot_model(prediction6.2a,type="pred",terms =c("State","Dataset"),
           show.values = T,
           show.p=T)+theme_classic()+
  labs(title="Local Bird Richness by Dataset Full GLM\n(State as random effect)",
       x="Dataset",
...
#now plotting diagnostic
plot(prediction6.2a)
plot_model(prediction6.2a, type="resid")
plot_model(prediction6.2a, type="diag")
qqnorm(residuals(prediction6.2a,type="deviance"))
qqline(resid(prediction6.2a))


```


```{r Prediction 6 full GLM Quassi-Poisson plots, message=FALSE, warning=FALSE}

#Plotting model
plot_model(prediction6.2b,show.values = T,show.p=T,show.intercept = T)+theme_classic()+
  labs(title="GLM All Factors Contributing to Local Richness")

#extracting r2
r2.c=round(r.squaredGLMM(prediction6.2b)[1,2],digits=4)
r2.m=round(r.squaredGLMM(prediction6.2b)[1,1],digits=4)

#now plotting the effect with richness on the y-axis
plot_model(prediction6.2b,type="pred",terms ="Dataset",
           show.values = T,
           show.p=T)+theme_classic()+
  labs(title="Local Bird Richness by Dataset Full GLM\n(State as random effect)",
       x="Dataset",
...
#now plotting the impact of the random effect of state
plot_model(prediction6.2b,type="re",
           show.values = T,
           show.p=T)+theme_classic()+
  labs(title="Random Effect of State Full GLM\n(BBA to BBS)",
       y="Rate Ratio (BBA to BBS)",
...
#now plotting predict with state
plot_model(prediction6.2b,type="pred",terms =c("State","Dataset"),
           show.values = T,
           show.p=T)+theme_classic()+
  labs(title="Local Bird Richness by Dataset Full GLM\n(State as random effect)",
       x="Dataset",
...
#now plotting diagnostic
#plot(prediction6.2b)
plot_model(prediction6.2b, type="resid")
plot_model(prediction6.2b, type="diag")
qqnorm(residuals(prediction6.2b))
qqline(resid(prediction6.2b))
...
**Checking for collinearity among predictors of full GLMM**
```{r Prediction 8 Pearson, message=FALSE, warning=FALSE}
...
#subsetting data
corey <- full_d %>%
  dplyr::select(Precipitation,  #precipitation
                Temp, #summer temp
                Elev_Mean, #mean elevation
                Elev_Range, #elevation range
                Het_Count2006, #land-class richness
                Shannon2006, #land-class diversity
                standard.effort, #sampling effort
                ) %>%
  rename(Rain = Precipitation,
         Elevation = Elev_Mean,
         `Elevation Range` = Elev_Range,
         `LC Richness` = Het_Count2006,
         `LC Diversity` = Shannon2006,
...
corrplot(call, method = "color",
         ...
         tl.col = "black", tl.srt = 90, tl.cex=1.2, # Text label color and rotation
...
#Running model
prediction8.1 <- glm(Atlas2_Total ~ logEffort, # response and predictor
                     data=full_d, # data
...
tab_model(prediction8.1,
          title="Poisson GLM Local Bird Richness by Effort (in hours)",
          string.pred = "Coefficient",
          string.ci = "95% CIs",
          string.p = "P-Value",
          show.se = T,
...
#linear model not poisson
prediction8.1b <- lm(Atlas2_Total ~ logEffort, # response and predictor
                     data=full_d)

## table of model
tab_model(prediction8.1b,
          title="LM Local Bird Richness by Effort (in hours)",
          string.pred = "Coefficient",
          string.ci = "95% CIs",
          string.p = "P-Value",
          show.se = T,
...
```{r Prediction 8 GLMM By State, message=FALSE, warning=FALSE}

#MA
prediction8.ma <- full_d %>%
  filter(State=="MA") %>%
  glm(logRichness ~ standard.effort*Dataset
                       +scale(Precipitation)  #precipitation
                       +scale(Temp)  #summer temperature
                       +scale(Elev_Mean)  #elevation mean in block/route
                       +scale(Elev_Range)  #elevation range in block/route
...
#table of model
tab_model(prediction8.ma,
          title="Local Bird Richness by Effort Full GLM MA",
          string.pred = "Coefficient",
          string.ci = "95% CIs",
          string.p = "P-Value",
          show.se = T,
...
#table of model
tab_model(prediction8.mi,
          title="Local Bird Richness by Effort Full GLM MI",
          string.pred = "Coefficient",
          string.ci = "95% CIs",
          string.p = "P-Value",
          show.se = T,
...
#table of model
tab_model(prediction8.ny,
          title="Local Bird Richness by Effort Full GLM NY",
          string.pred = "Coefficient",
          string.ci = "95% CIs",
          string.p = "P-Value",
          show.se = T,
...
#table of model
tab_model(prediction8.pa,
          title="Local Bird Richness by Effort Full GLM PA",
          string.pred = "Coefficient",
          string.ci = "95% CIs",
          string.p = "P-Value",
          show.se = T,
...
#table of model
tab_model(prediction8.vt,
          title="Local Bird Richness by Effort Full GLM VT",
          string.pred = "Coefficient",
          string.ci = "95% CIs",
          string.p = "P-Value",
          show.se = T,
...
```{r Prediction 8 state GLM Diagnostics, message=FALSE, warning=FALSE}
...
```{r Prediction 8 Full GLMM,message=FALSE,warning=FALSE}

#Running GLMM Poisson
prediction8.2 <- glmer(Atlas2_Total ~ standard.effort*Dataset   # interaction of data source and effort
                       +scale(Precipitation)  #precipitation
                       +scale(Temp)  #summer temperature
                       +scale(Elev_Mean)  #elevation mean in block/route
                       +scale(Elev_Range)  #elevation range in block/route
                       +scale(Het_Count2006)  # land-class richness
                       +scale(Shannon2006) # land-class diversity (Shannon-Wiener index)
                       +(1|State), #state random effect
                       data=full_d, # data
                       family="poisson") #poisson error structure
...
tab_model(prediction8.2,
          title="Local Bird Richness by Effort Full GLMM Poisson",
          string.pred = "Coefficient",
          string.ci = "95% CIs",
          string.p = "P-Value",
          show.se = T,
...
#Adjusting for overdispersion
kable(tidy_quasi(prediction8.2))

#---

#Running LMM 
prediction8.2a <- lmer(logRichness ~ standard.effort*Dataset   # interaction of data source and effort
                       +scale(Precipitation)  #precipitation
                       +scale(Temp)  #summer temperature
                       +scale(Elev_Mean)  #elevation mean in block/route
                       +scale(Elev_Range)  #elevation range in block/route
...
#table of model
tab_model(prediction8.2a,
          title="Local Bird Richness by Effort Full LMM",
          string.pred = "Coefficient",
          string.ci = "95% CIs",
          string.p = "P-Value",
          show.se = T,
...
#using the glmmTMB pacakge to calculate quasi-possion (i.e. negative binomial 1)
prediction8.2b <-glmmTMB::glmmTMB(Atlas2_Total ~ standard.effort*Dataset   # interaction of data source and effort
                       +scale(Precipitation)  #precipitation
                       +scale(Temp)  #summer temperature
                       +scale(Elev_Mean)  #elevation mean in block/route
                       +scale(Elev_Range)  #elevation range in block/route
...
#table of model
tab_model(prediction8.2b,
          title="Local Bird Richness by Effort Full GLMM Quasi-Poisson",
          string.pred = "Coefficient",
          string.ci = "95% CIs",
          string.p = "P-Value",
          show.se = T,
...
**Plotting full GLMM Poisson**

```{r Prediction 8 Full GLMM Plots, message=FALSE, warning=FALSE}

#rsquared
r2.c=round(r.squaredGLMM(prediction8.2)[1,2],digits=4)
r2.m=round(r.squaredGLMM(prediction8.2)[1,1],digits=4)
...
plot_model(prediction8.2,
           ...
           show.p=T)+
  theme_classic()+
  geom_label(aes(x=5,y=3,
                 label=paste("R2 Marginal=",r2.m,
                             "\nR2 Conditional=",r2.c)),col="black")+
  labs(title="GLMM Poisson All Factors Contributing to Local Richness")+
  geom_hline(yintercept = 1,linetype="dashed")
  

#plotting random effect
plot_model(prediction8.2, type="re")

#plot diagnostics
plot_model(prediction8.2, type="diag")
plot(prediction8.2)

qqnorm(residuals(prediction8.2,type="deviance"))
qqline(resid(prediction8.2))
...
**Plotting full LMM not Poisson**

```{r Prediction 8a Full LMM Plots, message=FALSE, warning=FALSE}

#rsquared
r2.c=round(r.squaredGLMM(prediction8.2a)[1,2],digits=4)
r2.m=round(r.squaredGLMM(prediction8.2a)[1,1],digits=4)

#Plotting model
plot_model(prediction8.2a,
           show.values = T,
           show.p=T)+
  theme_classic()+
  geom_label(aes(x=5,y=20,
                 label=paste("R2 Marginal=",r2.m,
                             "\nR2 Conditional=",r2.c)),col="black")+
  labs(title="LMM All Factors Contributing to Local Richness")+
  geom_hline(yintercept = 1,linetype="dashed")
  

#plotting random effect
plot_model(prediction8.2a, type="re")

#plot diagnostics
plot_model(prediction8.2a, type="diag")
plot(prediction8.2a)

qqnorm(residuals(prediction8.2a,type="deviance"))
qqline(resid(prediction8.2a))


#Plotting interaction of Dataset and Effort
plot(effect(c("standard.effort","Dataset"),prediction8.2a))

#Just effort
plot(effect("standard.effort",prediction8.2a))

#Just Dataset
plot(effect("Dataset",prediction8.2a))

#Precipitation
plot(effect("Precipitation",prediction8.2a))

#Temperature
plot(effect("Temp",prediction8.2a))

#Elevation
plot(effect("Elev_Mean",prediction8.2a))

#Elevation Range
plot(effect("Elev_Range",prediction8.2a))

#Land-Class Richness
plot(effect("Het_Count2006",prediction8.2a))

#Land-Class Diversity
plot(effect("Shannon2006",prediction8.2a))


```

**Now looking at the Quassi-Poisson or negative binomial model**

```{r Prediction 8b GLMM Diagnostics}
#Plotting model
plot_model(prediction8.2b,
           show.values = T,
           show.p=T)+
  theme_classic()+
  geom_label(aes(x=5,y=3,
                 label=paste("R2 Marginal =",0.382,
                             "\nR2 Conditional =",0.413)),col="black")+
  labs(title="GLMM All Factors Contributing to Local Richness Quasi-Poisson")+
  geom_hline(yintercept = 1,linetype="dashed")

#plotting random effect
plot_model(prediction8.2b, type="re")

#plot diagnostics
plot_model(prediction8.2b, type="diag")
DHARMa::simulateResiduals(prediction8.2b)

qqnorm(resid(prediction8.2b))
qqline(resid(prediction8.2b))

#Plotting interaction of Dataset and Effort
plot(effect(c("standard.effort","Dataset"),prediction8.2b))

#Just effort
plot(effect("standard.effort",prediction8.2b))

#Just Dataset
plot(effect("Dataset",prediction8.2b))

#Precipitation
plot(effect("Precipitation",prediction8.2b))

#Temperature
plot(effect("Temp",prediction8.2b))

#Elevation
plot(effect("Elev_Mean",prediction8.2b))

#Elevation Range
plot(effect("Elev_Range",prediction8.2b))

#Land-Class Richness
plot(effect("Het_Count2006",prediction8.2b))

#Land-Class Diversity
plot(effect("Shannon2006",prediction8.2b))
...
tab_model(prediction8.3,
          title="LMM Difference Richness by Difference Effort\nMatched Sampling Units",
          string.pred = "Coefficient",
          string.ci = "95% CIs",
          string.p = "P-Value",
          show.se = T,
...
pred %>%
  ...
  labs(title="GLM Richness Difference by Difference Effort (BBA-BBS)\nGeographically Matched Pairs",
       ...
       y="Difference in Richness per Sampling Unit (BBA-BBS)")+
  theme(plot.title=element_text(hjust=0.5))
...
#running model
assumption.landrich <- lmer(Atlas2_Total ~ Het_Count2006
                             + as.factor(Dataset)
                             + (1|State),
                             data=full_d)
...
#running model
assumption.landdiv <- lmer(Atlas2_Total ~ Shannon2006
                             + as.factor(Dataset)
                             + (1|State),
                             data=full_d)
