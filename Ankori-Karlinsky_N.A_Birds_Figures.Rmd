...
prediction8.1 <- glm(Atlas2_Total ~ logEffort*Dataset,
...
prediction8.data1 <- Matched %>%
  mutate(Richness_Diff = ifelse(BBSID==lag(BBSID),lag(log10(Atlas2_Total))-log10(Atlas2_Total),NA),
...
b<- pred %>%
  ...
  annotate("text",x=0.25,y=-1,
           label=paste("R^2 == 0.41"),parse=T)+
  annotate("text",x=0.25,y=-1.15,
           label=paste("                Y = -0.069+0.146X"))+
  annotate("text",x=0.25,y=-1.3,
           label=paste("p < 0.001"),parse=T)+
  theme_classic()+
  coord_cartesian(xlim=c(-1.5,2),ylim=c(-1.5,0.5))+
  scale_y_continuous(breaks=seq(-1.5,0.5,0.5))+
  scale_x_continuous(breaks=seq(-1.5,2,1))+
  labs(title="b",
       x="Difference in log sampling effort",
       y="Difference in log local richness",
       color=NULL)+
  theme(plot.title=element_text(size=18,face="bold"),
        axis.text.x = element_text(size=16),
        axis.title.x = element_text(face="bold",size=17,vjust=-1,hjust=0.5),
...
assumption.landrich <- glm(Atlas2_Total ~ Het_Count2006
                             + as.factor(Dataset),
                             data=predicts1.data,
                             family=poisson)
...
predicts1<- predict.glm(assumption.landrich,se.fit=T,type="response")
...
assumption.landdiv <- glm(Atlas2_Total ~ Shannon2006
                             + as.factor(Dataset),
                             data=predicts1.data,
                             family=poisson)
...
#predictions from model fit to then graph
predicts1b <- predict.glm(assumption.landdiv,se.fit=T,type="response")
